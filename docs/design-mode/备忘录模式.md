...
![备忘录模式，图片来自 refactoringguru.cn](https://gitee.com/laoyouji1018/images/raw/master/img/20210719225426.png)
...
![Photoshop 历史记录](https://gitee.com/laoyouji1018/images/raw/master/img/20210719225432.png)

## 案例场景模拟

![场景模拟；系统发布上线配置回滚](https://gitee.com/laoyouji1018/images/raw/master/img/20210719225436.png)
...
![备忘录模式模型结构](https://gitee.com/laoyouji1018/images/raw/master/img/20210719225446.png)
...
- 在以上的实现中我们是将配置模拟存放到内存中，如果关机了会导致配置信息丢失，因为在一些真实的场景里还是需要存放到数据库中。那么此种存放到内存中进行回复的场景也不是没有，比如；`Photoshop`、运营人员操作`ERP`配置活动，那么也就是即时性的一般不需要存放到库中进行恢复。另外如果是使用内存方式存放备忘录，需要考虑存储问题，避免造成内存大量消耗。
