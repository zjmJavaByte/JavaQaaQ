...
![访问者模式，图片来自 refactoringguru.cn](https://gitee.com/laoyouji1018/images/raw/master/img/20210719230708.png)

访问者要解决的核心事项是，在一个稳定的数据结构下，例如用户信息、雇员信息等，增加易变的业务访问逻辑。为了增强扩展性，将这两部分的业务解耦的一种设计模式。

![美女吃冰激凌](https://gitee.com/laoyouji1018/images/raw/master/img/20210719230724.png)

说白了访问者模式的核心在于同一个事物不同视角下的访问信息不同，比如一个`美女`手里拿个`冰激凌`。小朋友会注意冰激凌，大朋友会找自己喜欢的地方观测敌情。

## 案例场景模拟

![WechatIMG61](https://gitee.com/laoyouji1018/images/raw/master/img/20210720225301.png)
...
![访问者模式模型结构](https://gitee.com/laoyouji1018/images/raw/master/img/20210719230851.png)
...
public class DataView {
    ...
    public void show(Visitor visitor) {
        for (User user : userList) {
            user.accept(visitor);
            //这个方法可以这样理解：
            //1、visitor告诉user，我对你的关注点放在我包里的纸条上
            //2、如果想知道你可以自己去看 （调用的是 visitor中的accept方法）
        }
    }
...
- 另外在实现的过程可能你可能也发现了，定义抽象类的时候还需要等待访问者接口的定义，这样的设计首先从实现上会让代码的组织变得有些难度。另外从设计模式原则的角度来看，违背了`迪米特原则`(**校长、老师、学生三者之和直接关联的人交流**)，也就是最少知道原则。因此在使用上一定要符合场景的运用，以及提取这部分设计思想的精髓。
